...
extension ScheduleVC: AddTaskDelegate {
   
    

    func didAddTask(name: String, description: String, date: Date) {
        // Create a new task
        let newTask = Schedule(context: context)
        newTask.name = name
...
extension ScheduleVC {
    ...
    func getAllItems() {
        do {
            tasks = try context.fetch(Schedule.fetchRequest())
    ...
    func updateItem(item: Schedule, newName: String, newDescription: String) {
        item.name = newName
